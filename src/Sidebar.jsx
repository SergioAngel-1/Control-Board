...
export default function Sidebar({
  ...
  onShowDashboard, sidebarOpen, searchQuery, onSearchChange, onCreateProject,
      ...
      >
        ...
        <div className="px-3 pt-2 pb-1">
          <div className="flex items-center gap-2 px-2 py-1.5 rounded-sm bg-surface border border-border text-xs text-text-tertiary">
            ...
            />
            <span className="text-[9px] text-text-tertiary bg-surface-raised px-1 py-0.5 rounded">⌘K</span>
          </div>
        </div>
        <nav className="flex-1 overflow-y-auto scrollbar-thin py-1">
          ...
          {categories.map(cat => (
            <NavSection
              key={cat.id || cat}
              category={cat}
              projects={projectsByCategory[cat.name || cat] || []}
              activeProjectId={activeProjectId}
              collapsed={catCollapsed[cat.name || cat]}
              onToggle={() => onToggleCategory(cat.name || cat)}
